...
export class Scene extends React.Component<
    ...
    private createBubbleSpray(position: BABYLON.Vector3, shapeCount: number) {
        ...
        bubbleSpray.updateParticle = (particle) => {
            particle.position.addInPlace(particle.velocity); // ¥Êï£
            const rise = 0.001;
            particle.position.x += (direction.x * rise); // ‰∏äÂx
            particle.position.y += (direction.y * rise); // ‰∏äÂy 
        ...
        img.onload = () => {
            ...
            const textNodes = CommonUtility.sort(pixels, p => p.brightness).reverse()
                ...
                .map((p, i) => {
                    ...
                    return {
                        position: position, // ùÂ‰ΩçÁΩÆ
                        scale: CommonUtility.getRandomNumberInRange(0.005, 0.01, 3), // ßÂà∂üÂ∫¶
                        translateVector: new BABYLON.Vector3(0, 0, position.z < initialZ ? 1 : -1) // ùÂπÂ
                    };
                });
